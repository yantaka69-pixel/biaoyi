...
function parseAgentSectionMarkdown(markdown) {
  ...
  for (const line of lines) {
    const startMatch = /^\s*<!--\s*biaoyi-section-start\s+id="([^"]+)"[^>]*-->\s*$/.exec(line);
    if (startMatch) {
      if (currentId) {
    ...
    const endMatch = /^\s*<!--\s*biaoyi-section-end\s+id="([^"]+)"\s*-->\s*$/.exec(line);
    if (endMatch) {
...
- technical-plan.md：当前技术方案正文，包含章节标题、section id 和 biaoyi-section-start / biaoyi-section-end 标记。
...
- 保留原章节结构，不新增、删除或重排章节。
- 正文修改范围限定在 biaoyi-section-start 和 biaoyi-section-end 标记之间。
...
- 用户可见正文中不出现“原方案”“来源段”“用户原文”或类似过程性表述。`;
  ...
  function renderAgentTechnicalPlanOutline(items, sectionIndex, level = 1, lines = []) {
    for (const item of items || []) {
      ...
      lines.push(`<!-- biaoyi-section-start id="${escapeSectionAttribute(id)}" title="${escapeSectionAttribute(title)}" -->`);
      lines.push(section.originalContent);
      lines.push(`<!-- biaoyi-section-end id="${escapeSectionAttribute(id)}" -->`);
    }
    return lines;
  }
...
- technical-plan.md：当前技术方案正文全文，包含章节标题、section id 和 biaoyi-section-start / biaoyi-section-end 标记。
