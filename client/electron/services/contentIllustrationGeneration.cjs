...
const GENERATED_ILLUSTRATION_PATTERN = /<!-- biaoyi-illustration:start\b[^>]*-->[\s\S]*?<!-- biaoyi-illustration:end -->/gi;
...
function buildGeneratedIllustrationMarkdown(planItem) {
  ...
  if (!body) return '';
  return `<!-- biaoyi-illustration:start id="${planItem.item_id}" -->\n${body}\n<!-- biaoyi-illustration:end -->`;
}
