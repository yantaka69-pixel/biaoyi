...
function DeveloperTokenStatsWindow() {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    void window.biaoyi?.developerTokenStats.get()
      .then((nextStats) => {
    ...
    const unsubscribe = window.biaoyi?.developerTokenStats.onChanged((nextStats) => {
      setStats(nextStats);
    }) ?? (() => undefined);
  ...
  const resetStats = async () => {
    try {
      const nextStats = await window.biaoyi?.developerTokenStats.reset();
