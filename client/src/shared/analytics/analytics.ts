...
async function migrateLegacyClientId(config: ClientConfig) {
  ...
  try {
    const result = await window.biaoyi?.config.save(migratedConfig);
    if (result?.success) {
      removeLegacyClientId();
...
function getAnalyticsIdentity() {
  if (!identityPromise) {
    identityPromise = window.biaoyi?.config.load()
...
function getPlatform() {
  return window.biaoyi?.platform || window.biaoyiClient?.platform || '';
}

function getVersion() {
  if (!versionPromise) {
    versionPromise = window.biaoyi?.getVersion?.().catch(() => '') || Promise.resolve('');
  }
...
async function getAnalyticsLicenseSnapshot(): Promise<AnalyticsLicenseSnapshot> {
  ...
  try {
    const status = await window.biaoyi?.license?.getStatus();
    const value = status ? {
