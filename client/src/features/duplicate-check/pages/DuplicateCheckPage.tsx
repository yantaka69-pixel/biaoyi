...
function DuplicateCheckPage() {
  ...
  useEffect(() => {
    ...
    void window.biaoyi?.duplicateCheck.loadState()
      .then((state) => {
  ...
  useEffect(() => {
    ...
    void window.biaoyi?.duplicateCheck.saveUiState({ step, activeAnalysisTab })
      .catch((error) => {
  ...
  useEffect(() => {
    const unsubscribe = window.biaoyi?.tasks?.onTaskEvent<unknown, unknown, DuplicateCheckWorkspaceState>((event) => {
  ...
  const startDuplicateAnalysis = (force = false) => {
    ...
    void window.biaoyi?.tasks?.startDuplicateAnalysis({ tenderFile: tenderFiles[0] || null, tenderFiles, bidFiles, force })
      .then(() => {
  ...
  const selectFiles = async (multiple: boolean, filePaths?: string[]) => {
    const selector = window.biaoyi?.file?.selectDuplicateCheckFiles;
  ...
  const persistSelectedFiles = async (nextTenderFiles: LocalFileSelection[], nextBidFiles: LocalFileSelection[], nextStep: DuplicateCheckStep = step) => {
    const saver = window.biaoyi?.duplicateCheck?.saveFiles;
    ...
    const state = await window.biaoyi.duplicateCheck.loadState();
    applyDuplicateCheckState(state);
    setStartingAnalysis(false);
    startedMetadataSignatureRef.current = null;
    return state;
  ...
  const resetFiles = () => {
    ...
    void window.biaoyi?.duplicateCheck.clear()
      .then(() => {
