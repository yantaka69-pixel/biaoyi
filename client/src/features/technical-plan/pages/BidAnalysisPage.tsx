...
function BidAnalysisPage({
  ...
  const saveConfig = async (nextTaskIds = draftSelectedTaskIds, closeDialog = true, nextBidSectionMode = draftBidSectionMode) => {
    ...
    await window.biaoyi?.technicalPlan.saveBidAnalysisConfig({ mode: nextMode, selectedTaskIds: normalizedTaskIds, bidSectionMode: nextBidSectionMode });
  ...
  const startBidAnalysisOnly = async (taskIds: string[] | undefined, nextTaskIds: string[], nextBidSectionMode: BidSectionMode) => {
    ...
    try {
      ...
      await window.biaoyi?.tasks.startBidAnalysis({
  ...
  const startAnalysis = async (taskIds?: string[], nextTaskIds = draftSelectedTaskIds, options: { skipCheck?: boolean; overrideMode?: BidSectionMode } = {}) => {
    ...
    if (!options.skipCheck) {
      try {
        const detection = await window.biaoyi?.technicalPlan.checkBidSections();
    ...
    if (nextBidSectionMode === 'multiple' && !selectedSectionTitle) {
      try {
        ...
        setSettingsOpen(false);
        await window.biaoyi?.tasks.startBidSectionExtraction({});
  ...
  const handleSectionSelect = async (sectionId: string) => {
    ...
    try {
      ...
      const result = await window.biaoyi?.technicalPlan.selectBidSection(selectedSection);
      if (!result?.success) {
        showToast(result?.message || '投标范围选择失败', 'error');
        return;
      }
      onConfigSaved(await window.biaoyi.technicalPlan.loadState());
