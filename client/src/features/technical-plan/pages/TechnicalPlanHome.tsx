...
function TechnicalPlanHome({ workflowKind, registerLeaveGuard, onSectionChange }: TechnicalPlanHomeProps) {
  ...
  const executeWorkflowSwitch = useCallback(async (targetWorkflowKind: TechnicalPlanWorkflowKind) => {
    if (!window.biaoyi?.technicalPlan.switchWorkflowKind) {
    ...
    try {
      ...
      await window.biaoyi.technicalPlan.switchWorkflowKind(targetWorkflowKind);
      const saved = await window.biaoyi.technicalPlan.loadState();
  ...
  useEffect(() => {
    ...
    void window.biaoyi?.ui?.setCurrentView({ section: workflowKind, step: state.step });
  ...
  const switchStep = async (step: TechnicalPlanStep) => {
    ...
    if (state.step === 'outline-generation' && step === 'global-facts') {
      const latestState = await window.biaoyi!.technicalPlan.loadState();
    ...
    window.biaoyi?.technicalPlan.updateStep(step).catch((error) => {
  ...
  useEffect(() => {
    if (!window.biaoyi?.tasks) {
      return;
    }

    const unsubscribe = window.biaoyi.tasks.onTaskEvent<typeof state>((event) => {
    ...
    window.biaoyi.tasks.getActiveTasks().catch((error) => {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    window.biaoyi?.technicalPlan.readTenderMarkdown().then((markdown) => {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    window.biaoyi?.technicalPlan.readOriginalPlanMarkdown().then((markdown) => {
  ...
  const saveChapterContent = async (item: OutlineItem, content: string) => {
    ...
    const saved = await window.biaoyi?.technicalPlan.saveChapterContent({ nodeId: item.id, content });
  ...
  const resetTechnicalPlan = async () => {
    ...
    try {
      const result = await window.biaoyi?.technicalPlan.clear();
  ...
  const saveContentGenerationOptions = async (contentGenerationOptions: ContentGenerationOptions) => {
    const saved = await window.biaoyi?.technicalPlan.saveContentGenerationOptions(contentGenerationOptions);
    setState((prev) => ({ ...prev, ...(saved || {}), contentGenerationOptions }));
  };

  const saveGlobalFacts = async (globalFacts: GlobalFactGroupState[]) => {
    const saved = await window.biaoyi?.technicalPlan.saveGlobalFacts(globalFacts);
    setState((prev) => ({ ...prev, ...(saved || {}), globalFacts }));
  };

  const saveGlobalFactsConfig = async (globalFactsMode: GlobalFactsMode) => {
    const saved = await window.biaoyi?.technicalPlan.saveGlobalFactsConfig({ globalFactsMode });
    setState((prev) => ({ ...prev, ...(saved || {}), globalFactsMode }));
  };

  const saveOutline = async (request: SaveOutlineRequest) => {
    const saved = await window.biaoyi?.technicalPlan.saveOutline(request);
    setState((prev) => {
  ...
  const saveOutlineSelection = async (request: SaveOutlineSelectionRequest) => {
    await window.biaoyi?.technicalPlan.saveOutlineSelection(request);
  };

  const openBidTemplate = async () => {
    const result = await window.biaoyi?.technicalPlan.openBidTemplate();
    if (!result?.success) {
  ...
  const saveOutlineConfig = async (config: {
    ...
    await window.biaoyi!.technicalPlan.saveOutlineConfig(config);
    setState((prev) => ({
      ...prev,
