...
function RejectionCheckPage() {
  ...
  function persistRejectionState(partial: RejectionCheckWorkspacePatch, fallbackMessage: string) {
    void window.biaoyi?.rejectionCheck.updateState(partial)
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    void window.biaoyi?.rejectionCheck.loadState()
      .then((state) => {
  ...
  useEffect(() => {
    ...
    void window.biaoyi?.rejectionCheck.saveUiState({
      activeDocumentTab,
      step,
      activeResultTab,
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.biaoyi.tasks.onTaskEvent<unknown, RejectionCheckWorkspaceState>((event) => {
    ...
    void window.biaoyi.tasks.getActiveTasks()
      .then((tasks) => {
  ...
  async function importParsedDocument(role: RejectionDocumentRole, filePaths?: string[]) {
    ...
    try {
      const importer = window.biaoyi?.rejectionCheck.importDocument;
      ...
      applyWorkspaceState(await window.biaoyi.rejectionCheck.loadState());
  ...
  async function readTenderFromTechnicalPlan() {
    ...
    if (!window.biaoyi?.rejectionCheck?.importTenderFromTechnicalPlan) {
    ...
    try {
      ...
      const result = await window.biaoyi.rejectionCheck.importTenderFromTechnicalPlan();
      if (!result?.success) {
  ...
  function removeDocument(role: RejectionDocumentRole, documentId?: string) {
    ...
    setBusy('remove');
    void window.biaoyi?.rejectionCheck.removeDocument(role, documentId)
      .then(() => window.biaoyi.rejectionCheck.loadState())
      .then((state) => {
  ...
  async function startInvalidBidAndRejectionItemsExtraction(signature: string) {
    ...
    try {
      const starter = window.biaoyi?.tasks.startRejectionItemsExtraction;
  ...
  function resetWorkspace() {
    ...
    void window.biaoyi?.rejectionCheck.clear()
      .then(() => {
  ...
  async function saveCustomCheckItems() {
    ...
    const saveUiState = window.biaoyi?.rejectionCheck.saveUiState;
  ...
  async function startChecks(options: RejectionCheckOptions = checkOptions, runOptions: RejectionCheckOptions = options) {
    ...
    try {
      const starter = window.biaoyi?.tasks.startRejectionCheck;
      if (typeof starter !== 'function') {
        throw new Error('后台任务接口尚未加载，请重启应用后重试');
      }

      await window.biaoyi?.rejectionCheck.saveUiState({
