...
function createDefaultTemplateName(date = new Date()): string {
  ...
  return `biaoyi-${year}-${month}-${day}-${hour}${minute}${second}`;
}
...
function ExportFormatPage({ mode = 'create', templateId = null, onBack }: ExportFormatPageProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const fonts = await window.biaoyi?.systemFonts?.list?.();
    ...
    (async () => {
      ...
      try {
        if (mode === 'edit') {
          ...
          const template = await window.biaoyi?.templates.get(templateId);
          if (!template) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const template = currentTemplateId
        ? await window.biaoyi?.templates.update(currentTemplateId, nextConfig)
        : await window.biaoyi?.templates.create(nextConfig);
      if (!template) {
  ...
  const handleExportTest = useCallback(async () => {
    ...
    try {
      const technicalPlan = await window.biaoyi?.technicalPlan.loadState();
        ...
        {mode === 'create' ? (
          ...
          >
            <div className="settings-row-copy">
              ...
              <span>默认按 biaoyi-YYYY-MM-DD-HHmmss 格式生成，保存后显示在“我的模板”中。</span>
            </div>
