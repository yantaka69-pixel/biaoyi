...
function collectContentMetrics(content: string) {
  ...
  return {
    ...
    importedAssets: countMatches(text, /biaoyi-asset:\/\/imported-images/gi),
...
function KnowledgeBasePage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.biaoyi?.knowledgeBase.onEvent(({ document }) => {
  ...
  const handleFolderDrop = async (event: DragEvent<HTMLElement>, folderId: string) => {
    ...
    try {
      const result = payload.kind === 'folder'
        ? await window.biaoyi?.knowledgeBase.reorderFolder(payload.folderId, folderId, position)
        : await window.biaoyi?.knowledgeBase.moveDocument(payload.documentId, folderId, null, 'after');
      if (!result?.success) {
  ...
  const handleDocumentDrop = async (event: DragEvent<HTMLElement>, document: KnowledgeDocument) => {
    ...
    setDragSaving(true);
    try {
      const result = await window.biaoyi?.knowledgeBase.moveDocument(dragPayload.documentId, document.folder_id, document.id, position);
      if (!result?.success) {
  ...
  const loadAnalysis = async (documentId: string, options?: { silent?: boolean }) => {
    try {
      const data = await window.biaoyi?.knowledgeBase.readAnalysis(documentId);
  ...
  const createFolder = async () => {
    ...
    try {
      ...
      const folder = await window.biaoyi?.knowledgeBase.createFolder(name.trim());
      if (!folder) return;
  ...
  const uploadDocuments = async () => {
    ...
    try {
      setLoading(true);
      const result = await window.biaoyi?.knowledgeBase.uploadDocuments(activeFolder.id);
      if (!result?.success) {
  ...
  const renameFolder = async (folderId: string, currentName: string) => {
    ...
    try {
      const folder = await window.biaoyi?.knowledgeBase.renameFolder(folderId, name);
      if (!folder) return;
      setIndex((prev) => ({
        ...prev,
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (deleteConfirm.type === 'folder') {
        ...
        const result = await window.biaoyi?.knowledgeBase.deleteFolder(folderId);
        ...
        const result = await window.biaoyi?.knowledgeBase.deleteDocument(document.id);
  ...
  const retryDocument = async (document: KnowledgeDocument) => {
    ...
    try {
      const result = await window.biaoyi?.knowledgeBase.retryDocument(document.id);
      if (result?.document) {
  ...
  const openDocument = async (document: KnowledgeDocument, mode: KnowledgeViewer['mode']) => {
    ...
    try {
      if (mode === 'markdown') {
        ...
        const markdown = await window.biaoyi?.knowledgeBase.readMarkdown(document.id);
        ...
        const items = await window.biaoyi?.knowledgeBase.readItems(document.id);
  ...
  const startMatching = async (targetDocument = viewer?.document, options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const result = await window.biaoyi?.knowledgeBase.startMatching(targetDocument.id);
      if (!options?.silent) {
